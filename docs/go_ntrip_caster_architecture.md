...
# 6. RTCM Framing（先于 Fanout）

关键点：

-   TCP 是字节流协议，不保证一次 `Read()` 就得到一个完整 RTCM 包。
-   Source 读循环必须先做 RTCM3 framing，再把完整包送入广播链路。
-   Fanout 层的输入单位应是“完整 RTCM 包”，而不是任意 TCP 读块。

RTCM3 典型结构：

    0xD3 | length(10bit) | payload | CRC(3B)
...
-   `readLoop()` 不要把 `Conn.Read()` 返回的原始切片直接广播给所有客户端。
-   如果后续要加 CRC 校验，可放在 framing 阶段完成。
-   framing 失败、字节流错位、异常丢字节时，要有可观测日志与计数器。

------------------------------------------------------------------------

# 7. RTCM Fanout 架构（核心设计）
...
-   慢客户端踢除逻辑建议加防抖标记，避免在广播高峰期重复启动大量 `KickSlowConsumer()` goroutine。
...
-   如果底层连接是 `*net.TCPConn`，建议设置 `SetNoDelay(true)`，降低小包转发延迟。

------------------------------------------------------------------------

# 8. Mountpoint Manager
